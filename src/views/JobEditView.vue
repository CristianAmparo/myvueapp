...
const handleSubmit = async () => {
    ...
    if (!job.title || !job.type || !job.location ||!job.description || !job.salary || !job.company.name || !job.company.description || !job.company.contactEmail || !job.company.contactPhone) {
        ...
        try {
            await axios.put(`http://localhost:5000/jobs/${id}`, updateJob)
            alert(`'Job Updated '${id}`)
            router.push(`/jobs/${id}`)
            job.title = ''
            job.type = ''
            job.location = ''
